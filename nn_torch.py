...
def optimize_saved(model, N, root_dir, optimizer,
    ...
    if loss_file:
        if os.path.isfile(loss_file):
            ...
            losses = np.array([])
    with tqdm.tqdm(total=min(N*len(d),tMax*batchsize*N), dynamic_ncols=True,smoothing=0.01) as pbar:
        k0 = len(losses)
        k = k0
...
def overtrain(model,root_dir,optimizer,batchsize=20,clip=np.inf,smooth_display=0.9,loss_file=None,tMax=np.inf,smooth_l = 0, device='cpu'):
    d = dead_leaves_dataset(root_dir)
    dataload = DataLoader(d,batch_size=batchsize,shuffle=True,num_workers=6)
    print('starting optimization\n')
    with tqdm.tqdm(total=min(len(d),batchsize*tMax), dynamic_ncols=True,smoothing=0.01) as pbar:
...
def main(model_name,action,average_neighbors=False,device='cpu',weight_decay = 10**-3,epochs=1,lr = 0.001,tMax=np.inf,batchsize=20,time=5,n_neurons=10,kernel=3,im_size=5,average=False):
    ...
    if action == 'train':
        ...
        overtrain(model,data_folder,optimizer,batchsize=batchsize,clip=np.inf,smooth_display=0.9,loss_file=None,tMax=tMax,device=device)
    elif action == 'print':
        print(filename)
...
if __name__ == '__main__':
    ...
    parser.add_argument("-s","--tMax",type=int,help="maximum number of training steps",default=np.inf)
